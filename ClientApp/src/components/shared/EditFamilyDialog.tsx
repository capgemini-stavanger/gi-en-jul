import { Box, Button, Dialog, DialogActions, DialogTitle, Grid, IconButton, InputLabel, MenuItem, Select, TextField, Typography } from "@material-ui/core";
...
const EditFamilyDialog : FC<IEditFamilyDialog> = ({
    ...
    const onChangeGender = (index: number, e: React.ChangeEvent<{ name?: string | undefined; value: unknown }>) => {
        let newValue = e.target.value? parseInt(e.target.value as string) : Gender.Unspecified 
        setRecipientData((prev) => ({
            ...prev,
            familyMembers: [
    ...
    const newFamilyMember = () => {
        let newRecipient = {age: 0, months: 0, gender: 9, partitionKey: recipientData.rowKey, wish: ""} as PersonType;
                        ...
                        <Grid item>
                            ...
                            {recipientData.familyMembers?.map((familyMember : PersonType, fIndex: number) => (
                                <Grid container spacing={5} direction="row" alignItems="center" key={fIndex}>
                                    <Grid item>
                                        <InputLabel htmlFor="kjonn-header" shrink>Kjønn</InputLabel>
                                        <Select
                                            inputProps={{
                                                name: "kjonn",
                                                id: "kjonn-header",
                                            ...
                                            >
                                                {GENDERS.map((gender, gIndex) => {
                                                    return <MenuItem key={gIndex} value={gender.value.toString()}>{getGender(gender.value, familyMember.age)}</MenuItem>;
                                                })}
                                        </Select>
                                    </Grid>
                                    <Grid item>
