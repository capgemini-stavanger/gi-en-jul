...
const ConnectButton: React.FC<ConnectButtonProps> = ({
  ...
  const displayButton = () => {
    if (selectedConnection.giver != undefined && selectedConnection.recipient == undefined) {
      return (
        <Button onClick={connectGiverRecipient} className={classes.waitingButton}>
          Velg en familie å koble sammen med <br /> {selectedConnection.giver?.fullName}
        </Button>
      );
    }
    if (selectedConnection.giver == undefined && selectedConnection.recipient != undefined) {
          ...
          <Modal open={open}>
            <Box className={classes.modalStyle}>
              <Typography variant="h6" component="h2">
                Bekreft at du vil koble {selectedConnection.giver?.fullName} sammen med
                familienummer {""}
                {selectedConnection.recipient?.familyId}
              </Typography>
